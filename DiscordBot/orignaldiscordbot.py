...
def read_bot_token():
  with open('../Tokens/OrignalToken.txt', 'r') as file:
...
async def on_message(message):
  ...
  if 'hi' in content_lower:
    ...
    await message.channel.send(f"I am doing good <@!{message.author.id}>, how are you doing?")
    ...
    try:
      ...
      await message.channel.send("I didn't receive your response. If you want to talk, feel free to message me anytime!")
    else:
      good_words = [
...
@app_commands.describe(auto_delete="Would you like the messages to auto delete?")
@app_commands.choices(auto_delete=[
    discord.app_commands.Choice(name="True", value=1),
    discord.app_commands.Choice(name="False", value=2),
])
async def send_multible_messages(interation: discord.Interaction, how_many_times_to_send_message: int, message: str, channel: discord.TextChannel, auto_delete: discord.app_commands.Choice[int]):
  ...
  if (auto_delete.name == "True"):
    await channel.purge(limit=int(how_many_times_to_send_message))
  channel2 = bot.get_channel(MOD_LOG_CHANEL_ID)
...
async def poll(ctx: discord.Interaction, num_of_options: int, question: str, channel: discord.TextChannel):
  ...
  for i in range(num_of_options):
    ...
    embed.set_footer(text="react to the numbers to vote")
...
@bot.tree.command(name="test-command")
@app_commands.describe(mentions="Who do you want to mention with the message?(optional)")
async def send_announcement(interaction: discord.Interaction, mentions: discord.Role=None):
  ...
  await main_channel.send(mentions.mention)
